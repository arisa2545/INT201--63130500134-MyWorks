...
export function showMenus(arr) {
    ...
    //จะทำการเช็คก่อนว่ามีสถานะเป็น available รึป่าว ถ้าใช่จะเเสดงปุ่มเป็น Add To Cart เเละใส่ event ให้
    if(menu.menuStatus == 'available'){
      ...
      buttonAddEle.addEventListener('click', () => {
        ...
        // เพิ่ม newItem ลง carts ด้วย function addItem
        carts.addItem(newItem);
        // ใช้ function save เพื่ออัพเดตหรือเพิ่ม cookie เข้าไป
        carts.save();
  
      })

    // เเต่ถ้าไม่ใช่สถานะ available จะเเสดงเป็นปุ่ม Sols Out เเละไม่มีการ add event ให้ 
    } else {
